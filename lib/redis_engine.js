...
var RedisEngine = function(server, addresses) {
...
RedisEngine.prototype = {
  ...
  addClient: function(channel, clientId, presenceId, data, done) {
    ...
    self._server.debug('Registering client ? for presence channel ? ?', clientId, channel, presenceId);

    self._server._engine.clientExists(clientId, function (existsResult) {
      if(!existsResult) {
        self._server.debug('Will not register client ? for presence channel ? ?', clientId, channel, presenceId);
      ...
      self._shardManager.getShard(channel).evalsha.exec('subscribe_presence', [
        ...
        self._server._engine.clientExists(clientId, function (existsResult) {
          if(!existsResult) {
            self._server.debug('Client ? disappeared while registering for presence channel ? ?', clientId, channel, presenceId);
            ...
            self._server.debug('Registered new client ? for presence channel ? ? ?', clientId, channel, presenceId);

            done(null, false);
          } else if(results === 0) {
            self._server.debug('Registered initial client ? for presence channel ? ? ?', clientId, channel, presenceId);

            done(null, true);
          } else {
            self._server.debug('Failed to register client ? for presence channel ? ? ? ?', clientId, channel, presenceId, err, results);
          }
        });
      });
    });
  ...
  unsubscribe: function(channel, clientId, done) {
    ...
    client.get(key('pid', channel, clientId), function(err, presenceId) {
      if(presenceId === null) {
        self._server.debug('Will not deregister client ? for presence channel ? ?', clientId, channel, presenceId);

        return;
      }

      self._server.debug('Deregistering client ? for presence channel ? ?', clientId, channel, presenceId);
      ...
      self._shardManager.getShard(channel).evalsha.exec('unsubscribe_presence', [
        ...
        if(results === 1) {
          self._server.debug('Deregistered client ? for presence channel ? ?', clientId, channel, presenceId);

          if(done) done(null, presenceId, false);
        } else if(results === 0) {
          self._server.debug('Deregistered final client ? for presence channel ? ?', clientId, channel, presenceId);

          if(done) done(null, presenceId, true);
        } else {
          self._server.debug('Failed to deregister client ? for presence channel ? ? ? ?', clientId, channel, presenceId, err, results);
        }
      });
    });
  }
